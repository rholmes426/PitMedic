...
export async function renderMetricDetails(url:URL,env:WebsiteAnalyticsEnv & SearchConsoleEnv,now:Date,styles:string):Promise<string> {
  ...
  let content:string;
  if(google) {
    const data=await loadSearchConsoleData(env,now,{start,end,page:q.get('page')||undefined,query:q.get('query')||undefined,country:q.get('country')||undefined,device:q.get('device')||undefined});
    if(!data.available) content=`<div class="empty">${esc(data.message)}</div>`;
      ...
      const searchLink=(value:string|number,selectedMetric:string,dimension:string,label:string) => metricLink(value,detailUrl(selectedMetric,start,end,{...Object.fromEntries([...q].filter(([key])=>!['metric','start','end'].includes(key))),[dimension]:label}));
      const searchTable=(title:string,rows:SearchMetricRow[],dimension?:string) => `<section class="panel"><h2>${title}</h2><div class="table-wrap"><table><thead><tr><th>${dimension==='query'?'Query':'Dimension'}</th><th>Clicks</th><th>Impressions</th><th>CTR</th><th>Position</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.label)}</td><td class="number">${dimension?searchLink(r.clicks,'google-clicks',dimension,r.label):num(r.clicks)}</td><td class="number">${dimension?searchLink(r.impressions,'google-impressions',dimension,r.label):num(r.impressions)}</td><td class="number">${dimension?searchLink(`${num(r.ctr*100)}%`,'google-ctr',dimension,r.label):`${num(r.ctr*100)}%`}</td><td class="number">${dimension?searchLink(r.position?num(r.position):'—','google-position',dimension,r.label):r.position?num(r.position):'—'}</td></tr>`).join('')||'<tr><td colspan="5">No reported rows for this selection.</td></tr>'}</tbody></table></div></section>`;
      content=`<section class="panel"><span class="eyebrow">${esc(labels[metric] || metric)}</span><h2>${pending?'Awaiting Google data':esc(String(value))}</h2><p>${data.daily.some(r=>r.status!=='final')?'This range includes preliminary or awaiting data. Google may revise it.':'Finalized date range.'} Lower average position is better.</p><p>Google omits some queries for privacy. Page impressions can exceed site impressions when several pages appear in one search; these breakdowns may not sum to the site total.</p></section>${searchTable('Landing pages',data.pages,'page')}${searchTable('Visible search queries',data.queries,'query')}${searchTable('Countries',data.countries||[],'country')}${searchTable('Devices',data.devices||[],'device')}`;
    }
  } else {
  ...
  const today=google?new Intl.DateTimeFormat("en-CA",{timeZone:"America/Los_Angeles",year:"numeric",month:"2-digit",day:"2-digit"}).format(now):webRange(now,1).end;
  const previousDay=new Date(Date.parse(today)-86400000).toISOString().slice(0,10);
  const presetFilters=Object.fromEntries([...q].filter(([key])=>!['metric','start','end'].includes(key)));
  const presets:[string,string,string][]=[['Today',today,today],['Yesterday',previousDay,previousDay],['Last 7 days',new Date(Date.parse(today)-6*86400000).toISOString().slice(0,10),today],['Last 30 days',new Date(Date.parse(today)-29*86400000).toISOString().slice(0,10),today]];
  const shortcuts=presets.map(([label,from,to])=>`<a class="metric-link" href="${esc(detailUrl(metric,from,to,presetFilters))}"${start===from&&end===to?' aria-current="date"':''}>${label}</a>`).join(' · ');
