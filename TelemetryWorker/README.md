...
It temporarily retains the read-only lap benchmark endpoint for compatibility with already released PitMedic clients. The next app release no longer collects best laps or calls this endpoint. The endpoint accepts only simulator, track/layout, and car names; it never receives the user's lap time or identity.
